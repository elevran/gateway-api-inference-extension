...
func TestLoRASoftAffinityDistribution(t *testing.T) {
	...
	pods := []types.Pod{
		&types.PodMetrics{
			PodInfo: &k8s.PodInfo{NamespacedName: k8stypes.NamespacedName{Name: "affinity-pod"}},
			MetricsState: &backendmetrics.MetricsState{
				MaxActiveModels: 2,
		...
		&types.PodMetrics{
			PodInfo: &k8s.PodInfo{NamespacedName: k8stypes.NamespacedName{Name: "available-pod"}},
			MetricsState: &backendmetrics.MetricsState{
				MaxActiveModels: 2,
				ActiveModels:    map[string]int{},
			},
